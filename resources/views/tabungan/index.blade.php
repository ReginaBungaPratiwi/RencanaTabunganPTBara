...
<div class="container mt-4">
    <h2 class="mb-4">📋 Daftar Rencana Tabungan</h2>
    ...
    <a href="{{ route('tabungan.create') }}" class="btn btn-primary mb-3">➕ Tambah Tabungan</a>
    ...
    @else
        <div class="row">
            @foreach ($tabungans as $tabungan)
                <div class="col-md-4">
                    <div class="card mb-3">
                        <img src="{{ asset('storage/' . $tabungan->foto) }}" class="card-img-top" alt="Foto Tabungan" style="height: 200px; object-fit: cover;">
                        <div class="card-body">
                            <h5 class="card-title">{{ $tabungan->judul }}</h5>
                            <p class="card-text">Target: Rp{{ number_format($tabungan->target_nominal, 0, ',', '.') }}</p>
                            <p class="card-text">Tercapai: {{ $tabungan->tercapai ? '✅' : '❌' }}</p>
                            <p class="card-text"><small class="text-muted">Target Tanggal: {{ \Carbon\Carbon::parse($tabungan->target_tanggal)->format('d M Y') }}</small></p>
                            <a href="{{ route('tabungan.show', $tabungan->id) }}" class="btn btn-sm btn-info">Detail</a>
                        </div>
                    </div>
                </div>
            @endforeach
